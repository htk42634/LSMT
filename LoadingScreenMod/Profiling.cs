...
namespace LoadingScreenModTest
{
    public static class Profiling
    {
        ...
        internal static string FAILED, DUPLICATE, MISSING;

        internal static void Init()
        {
            Sink.builder.Length = 0;
            FAILED = L10n.Get(" (", L10n.FAILED, ")");
            DUPLICATE = L10n.Get(" (", L10n.DUPLICATE, ")");
            MISSING = L10n.Get(" (", L10n.MISSING, ")");
    ...
    internal sealed class LineSource : Source
    {
        ...
        static string Limit(string s) => s.Length <= 40 ? s : s.Substring(0, 40);
        protected internal override string CreateText() => sink.CreateText(IsLoading());
        internal void Add(string s) => sink.Add(s);
        internal void AddNotFound(string s) => Add(string.Concat(Limit(s), Profiling.MISSING));
        internal void AddFailed(string s) => Add(string.Concat(Limit(s), Profiling.FAILED));
        internal void AddDuplicate(string s) => Add(string.Concat(Limit(s), Profiling.DUPLICATE));
    }
    ...
    internal sealed class DualProfilerSource : Source
    {
        ...
        readonly string failedStr = L10n.Get(" ", L10n.FAILED, " ");
        readonly string missingStr = L10n.Get(" ", L10n.MISSING, " ");
        readonly string duplicatesStr = L10n.Get(" ", L10n.DUPLICATES);
        int state = 0;
        ...
        void AdjustName()
        {
            string s1 = failed == 0 ? string.Empty : string.Concat(failed.ToString(), failedStr);
            string s2 = notFound == 0 ? string.Empty : string.Concat(notFound.ToString(), missingStr);
            string s3 = duplicate == 0 ? string.Empty : string.Concat(duplicate.ToString(), duplicatesStr);
