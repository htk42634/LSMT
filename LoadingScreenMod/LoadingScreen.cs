    ...
    public sealed class LoadingScreen : DetourUtility<LoadingScreen>
    {
        ...
        public void SetFont()
        {
            try
            {
                ...
                list.Add(new Text(new Vector3(-1.3f,   0.7f,  10f), new DualProfilerSource(L10n.Get(L10n.SCENES_AND_ASSETS), 36)));
                list.Add(new Text(new Vector3(-0.33f, -0.52f, 10f), new SimpleProfilerSource(L10n.Get(L10n.MAIN), LoadingManager.instance.m_loadingProfilerMain)));
                list.Add(new Text(new Vector3(-0.33f, -0.62f, 10f), new SimpleProfilerSource(L10n.Get(L10n.SIMULATION), LoadingManager.instance.m_loadingProfilerSimulation)));
                list.Add(new Text(new Vector3(-0.12f,  0.7f,  10f), new LineSource(L10n.Get(L10n.ASSETS_LOADER), 2, LevelLoader.AssetsActive)));
        ...
        internal void SetProgress(float min, float max, int assetsCount, int assetsTotal, int beginMillis, int nowMillis)
        {
            ...
            if (assetsCount > 0 && nowMillis > beginMillis)
            {
                ...
                loader.Add(string.Concat((assetsCount * 1000f / (nowMillis - beginMillis)).ToString("G3"), L10n.Get(L10n.PER_SECOND)));
            }
        }
        ...
        static Mesh CreateQuads()
        {
            ...
            CreateQuad(-1.35f,  0.75f, 0.86f, 1.50f, vertices, triangles);
