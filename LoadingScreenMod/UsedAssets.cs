...
namespace LoadingScreenModTest
{
    internal sealed class UsedAssets : Instance<UsedAssets>
    {
        ...
        internal bool IsUsed(Package.Asset assetRef, CustomAssetMetaData.Type t1, CustomAssetMetaData.Type t2) =>
            allAssets[(int) t1].Contains(assetRef.fullName) || allAssets[(int) t2].Contains(assetRef.fullName);
        ...
        static void ReportMissingAssets<P>(HashSet<string> customAssets, CustomAssetMetaData.Type type) where P : PrefabInfo
        {
            try
            {
                bool reportMissing = Settings.settings.reportAssets | Settings.settings.hideAssets;
                ...
                foreach (string fullName in customAssets)
                    if (CustomDeserializer.FindLoaded<P>(fullName, tryName:false) == null)
                    {
                        ...
                        if (reportMissing)
                            Reports.instance.AddMissing(fullName, type);
                    }
            }
            catch (Exception e)
            {
